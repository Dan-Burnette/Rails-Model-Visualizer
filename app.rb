...
get '/visualize_repo' do
  ...
  begin
    ...
    @table_names_to_column_lines =
      ParseSchemaTables.call(repo.schema_file_content)
    ...
    @error_message =
      "Couldn't find the database schema for that repository! Looking for db/schema.rb."
    @attempted_url = repo_url
    erb :index
  rescue StandardError => error
    production? ? Rollbar.error(error, url: repo_url) : raise(error)
    @error_message =
      "Something went wrong visualizing that repository. I'll look into a fix."
    @attempted_url = repo_url
    erb :index
  end
end
...
def models_to_associations(repo)
  ...
  model_classes =
    model_file_contents.map { |contents| ParseClassName.call(contents) }

  result = {}
  model_file_contents.each_with_index do |file_content, i|
    class_name = model_classes[i]
    associations =
      ParseAssociations.call(model_classes, class_name, file_content)
